...
class Index extends Frontend
{
    ...
    public function index()
    {
        ...
        $navs = self::category('navs', 8, 6);   //首页导航，项目
        $advert_a = self::advert('A', 6, 80); //图文广告a
        $advert_b = self::advert('B', 6, 12);  //图文广告b
        $advert_c = self::advert('C', 6, 12); //图文广告c
        $advert_d = self::advert('D', 8, 8);  //图文广告d
        $advert_e = self::advert('E', 6, 6);  //图文广告e
        //友情链接
        $frend_url=  db::name('advert')->field('id,title,url,nofollow')->where('type', 2)->whereNull('deletetime')->where('switch', 1)->order('weigh desc')->select();
        ...
        $article = self::article('index', 8, 10); //首页文章
        $slide = db::name('advert')->field('id,title,url,image')->where('switch', 1)->where('flag', 'like', "%S%")->limit(6)->select(); //轮播图广告
        $help = db::name('help')->where('switch', 1)->whereNull('deletetime')->field('id,title')->select(); //帮助
    ...
    public function search()
    {
        ...
        if ($type == 'article') {
            $data = db::name('article')
                ...
                ->paginate(10, false, ['type' => 'defult', 'query' => ['type' => $type, 'keyword' => $keyword]]);
            $this->assign('data', $data);
        ...
        if ($type == 'project') {
            ...
            for ($i = 0; $i < count($p); $i++) {
                if ($p[$i]['city'] != null || $p[$i]['address']) {
                    ...
                    $name  = db::name('china')->where('id', $p[$i]['address'])->find();
                    $p[$i]['address'] = $name['areaname'];
                } else {
                    $p[$i]['city'] = "全国-全国各地";
                }
            }
            ...
            if ($flag == 'menu' || $flag == 'navs') { //首页左侧菜单栏的二级分类下的项目
                for ($j = 0; $j < count($menu_two); $j++) {
                    $menu_two[$j]['project'] = db::name('project')
                        ->field('id,name,image,category_id,price')
                        ->where('switch', 1)
                        ->whereNull('deletetime')
    ...
    public static function article($flag, $flag_number, $limit)
    {
        ...
        for ($i = 0; $i < count($category); $i++) {
            $category[$i]['content'] = db::name('article')
                ->field('id,title,image,category_id,views,desc,createtime')
    ...
    {
        $data = db::name('web_sm')->where('id', 1)->find();
        $this->assign('data', $data);
        //是否手机端访问
        if (request()->isMobile()) {
            return $this->view->fetch('mobile/index/lxwm');
        }
        return $this->view->fetch('index/lxwm');
    }
    //关于我们
    public function about()
    {
        ...
        //是否手机端访问
        if (request()->isMobile()) {
            return $this->view->fetch('mobile/index/about');
        }
    ...
    {
        $data = db::name('web_sm')->where('id', 2)->find();
        $this->assign('data', $data);
        //是否手机端访问
        if (request()->isMobile()) {
            return $this->view->fetch('mobile/index/mzsm');
        }
        return $this->view->fetch('index/mzsm');
    }
    //投诉删除
    public function tssc()
    {
        $data = db::name('web_sm')->where('id', 3)->find();
        $this->assign('data', $data);
        //是否手机端访问
        if (request()->isMobile()) {
            return $this->view->fetch('mobile/index/tssc');
        }

        return $this->view->fetch('index/tssc');
    }
