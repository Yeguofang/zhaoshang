<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:77:"D:\Project\zhaoshang\public/../application/index\view\menber\advert\list.html";i:1572765525;s:67:"D:\Project\zhaoshang\application\index\view\menber\common\head.html";i:1572765525;}*/ ?>
<!DOCTYPE html>
...
<script type="text/html" id="toolbarDemo">
  ...
  <button class="layui-btn" onclick="xadmin.open('发布广告','<?php echo url('menber/advert/add'); ?>',900,800)" >发布广告</button>
  <button class="layui-btn layui-btn-danger" lay-event="getCheckData">批量删除</button>
  </div>
</script>

<script type="text/html" id="barDemo">
  <a class="layui-btn layui-btn-xs" lay-event="edit" onclick="xadmin.open('发布文章','<?php echo url('menber/advert/edit'); ?>/{{d.id}}',900,800)">编辑</a>
...
<script>
	...
	layui.use(['table', 'common'], function () {
		...
		table.render({
			elem: '#advert'
			, url: "<?php echo url('menber/advert/list'); ?>"
		...
		table.on('toolbar(advert)', function (obj) {
			tools.delAll_ajax('post', "<?php echo url('/menber/advert/del'); ?>", table,obj);
		});
		...
		table.on('tool(advert)', function (obj) {
			var ids = obj.data.id;
			if (obj.event === 'del') {
				tools.del_ajax('post', "<?php echo url('/menber/advert/del'); ?>", ids, obj);
