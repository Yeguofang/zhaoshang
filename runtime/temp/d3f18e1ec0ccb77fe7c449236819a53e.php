<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:77:"D:\Project\zhaoshang\public/../application/index\view\menber\project\msg.html";i:1572765525;s:67:"D:\Project\zhaoshang\application\index\view\menber\common\head.html";i:1572765525;}*/ ?>
<!DOCTYPE html>
...
<script>
	...
	layui.use(['table', 'common'], function () {
		...
		table.render({
			elem: '#project'
			, url: "<?php echo url('menber/project/msg'); ?>"
		...
		table.on('toolbar(project)', function (obj) {
			tools.delAll_ajax('post', "<?php echo url('/menber/project/del_msg'); ?>", table,obj);
		});
		...
		table.on('tool(project)', function (obj) {
			var ids = obj.data.id;
			if (obj.event === 'del') {
				tools.del_ajax('post', "<?php echo url('/menber/project/del_msg'); ?>", ids, obj);
			} 
		});
	});
</script>
